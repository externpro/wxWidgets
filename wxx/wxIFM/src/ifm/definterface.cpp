...
void wxIFMDefaultPanelPlugin::OnDock(wxIFMDockEvent &event)
{
    ...
    if( event.GetDestinationType() == IFM_COMPONENT_PANEL )
    {
        ...
        int h;
        GetManager()->GetParent()->GetTextExtent(tab->m_tab->m_name, &tab->m_name_width, &h, 0, 0, &m_tab_config.m_font);
...
void wxIFMDefaultPanelPlugin::UpdateTabs(wxIFMComponent *panel)
{
    if( panel && panel->GetType() == IFM_COMPONENT_PANEL )
    {
        ...
        for( unsigned int i = 0; i < data->m_tabs.size(); ++i )
        {
            ...
            // calculate the width of the tabs name string
            GetManager()->GetParent()->GetTextExtent(tab->m_tab->m_name, &tab->m_name_width, 0, 0, 0, &m_tab_config.m_font);
            if( tab->m_name_width == 0 )
                tab->m_name_width = 20;
        }
...
void wxIFMDefaultPanelPlugin::OnPaintBg(wxIFMPaintEvent &event)
{
    ...
    */
    ...
    if( event.GetComponentType() == IFM_COMPONENT_PANEL || event.GetComponentType() == IFM_COMPONENT_PANEL_TAB )
    {
        ...
        dc.SetPen(wxPen(m_tab_config.m_tab_active_bg_b.GetColour()));
...
void wxIFMDefaultPanelPlugin::DrawTabs(wxDC &dc, wxIFMComponent *component, const wxRegion &region, const wxIFMTabArray &tabs)
{
    ...
    for( size_t i = 0; i < tabs.GetCount(); i++ )
    {
        ...
        if( tab->m_active )
        {
            ...
            dc.SetPen(m_tab_config.m_tab_active_bg_p);
...
void wxIFMDefaultPanelPlugin::DrawCaption(wxDC &dc, const wxRect &rect, const wxString &caption, int orientation) 
{
...
void wxIFMTabConfig::init()
{
    ...
    m_tab_active_bg_b = wxBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNHIGHLIGHT));
    m_tab_active_bg_p = wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNHIGHLIGHT));
    m_tab_inactive_bg_b = wxBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE));
    m_tab_inactive_bg_p = wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_BTNFACE));
    m_bg_brush = wxBrush(wxSystemSettings::GetColour(wxSYS_COLOUR_3DLIGHT));
    m_bg_pen = wxPen(wxSystemSettings::GetColour(wxSYS_COLOUR_3DLIGHT));
